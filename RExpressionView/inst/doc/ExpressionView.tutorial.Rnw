...
\begin{description}
	...
	\item[e, f] Zoom and pan modes allow you to restrict the view to a particular part of the gene expression data. They behave like the functions used in graphic programs.
	...
	\item[g, h] Modules are highlighted as the user moves the moves over the gene expression data. The two check boxes allow you to choose between highlighting all the parts of a module (Filling) or alternatively only the largest rectangle (Outline). You can also turn it off completely. For data sets with many modules, it can be helpful to restrict highlighting to Outline.
	...
	\item[k] Sets the visibility of the modules layer. Moving the slider to the left fades out the gene expression data, focusing on the Biclusters, while towards the opposite direction, the gene expression data moves to the foreground.
