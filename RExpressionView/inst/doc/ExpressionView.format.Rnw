...
code snipplets are in boxes with green frames.
...
\end{schema}
An EVF file contains a single \texttt{<evf>} tag. It has the following
parts:
\begin{schema}
...
This is the type of the \texttt{<summary>} tag:
\begin{schema}  
...
gene expression window in ExpressionView. The \texttt{<version>} tag
...
exported using the ExpressionView R package. \texttt{<xaxislabel>} and
\texttt{<yaxislabel>} are optional axis labels for the gene expression
plot. The last three tags (\texttt{<nmodules>}, \texttt{<ngenes>} and
\texttt{<nsamples>}) are required and give the number of modules,
...
viewer. The \texttt{<annotation>} tag should contain the name of the
...
The next tag within \texttt{<evf>} is \texttt{<genes>}, its type os
defined as
\begin{schema}  
...
from other tags in the document. \texttt{<name>} is typically the name
...
might contain any number of \texttt{<x>} tags, each defining an
...
file. The definition of the \texttt{<gene>} tag:
\begin{schema}
...
\end{schema}
The \texttt{<gene>} tags must have exactly the same subtags as the
ones given in the gene tags section. For example, assuming the
\texttt{<genetags>} example above we can have:
\begin{XML}
<gene>
  <id>1</id>
...
e.g. the ones that define the modules. \texttt{<name>} is an arbitrary
...
\texttt{<keggtags>} tag. The definition of \texttt{<gotags>}:
\begin{schema}
...
\end{schema}
\texttt{<id>} is a numeric id, starting from one, up to the total
...
Part of an EVF file with the \texttt{<gotags>} and
\texttt{<keggtags>} tags:
\begin{XML}
<gotags>
  <id>#</id>
...
the tags already listed above in the \texttt{<gotags>} and
\texttt{<keggtags>} tags:
\begin{schema}
