...
\DefineVerbatimEnvironment%
    {XML}{Verbatim}{fontsize=\small,frame=single,rulecolor=\color{green}}

\DefineVerbatimEnvironment%
    {schema}{Verbatim}{fontsize=\small,frame=single,rulecolor=\color{blue}}
...
ordering algorithm, written in GNU R, thay shuffles the rows and
columns of the gene expression matrix to make (potentially
overlapping) biclusters more visible. The second part is the
interactive tool, written in Adobe Flex. This runs in an Adobe Flash
enabled web browser. The user can export the ordered gene expression
matrix, with additional meta-data to a data file, that can be openened
by the Adobe Flash application. In this document we briefly discuss
...
data file. Parts of the schema file appear in blue boxes, EVF file
code snipplets are in green boxes.
...
the older 1.0 versions.

\begin{schema}
<xsd:element name="evf" type="evfType" />
\end{schema}
An EVF file contains a single \texttt{<evf>} tag, which has a following
parts:
\begin{schema}
...
The \texttt{<summary>} tag contains information about the data set, such
as the number of genes, samples, modules, etc. \texttt{<experimentdata>}
is for experiment meta-data, i.e. the lab where it was performed, 
the abstract of the related publication and possibly
more. \texttt{<genes>} and \texttt{<samples>} have the gene and sample
meta data. \texttt{<modules>} defines the biclusters. Finally
\texttt{<data>} contains the expression values themselves. Let us see
...
\end{schema}
The \texttt{<summary>} tag optionally contains the description of the
data file (\texttt{<description>} tag), this is displayed above the main
gene expression window. The \texttt{<version>} tag is required must be
\texttt{1.1} for files of the format we are discussing here. The
\texttt{<dataorigin>} tag is optional and has the value \texttt{eisa} for
modules generated with the ISA algorithm and exported using the
ExpressionView R package. \texttt{<xaxislabel>} and \texttt{<yaxislabel>}
are optional axis labels for the gene expression plot. The last three
tags (\texttt{<nmodules>}, \texttt{<ngenes>} and \texttt{<nsamples>}) are
required and give the number of modules, number of genes and number of
samples in the data set.

A sample EVF file header, together with the \texttt{<summary>} tag looks
like this:
\begin{XML}
...
The \texttt{<experimentdata>} tag is next, this contains the
...
\end{schema}
All fields are optional here: \texttt{<title>}, \texttt{<name>},
\texttt{<lab>}, \texttt{<abstract>}, \texttt{<url>},
\texttt{<annotation>}, \texttt{<organism>}. They are collected and
shown in the \emph{Experiment} tab in the interactive ExpressionView
viewer. The \texttt{<annotation>} tag shoudl contain the name of the
chip on which the experiment was performed. If the \texttt{<organism>}
tag contains \emph{Homo sapiens}, then ExpressionView links genes with
...
Here is an example for the \texttt{<experimentdata>} tag:
\begin{XML}
<experimentdata>
...
The next tag within \texttt{<evf>} is \texttt{<genes>}:
\begin{schema}  
...
\end{schema}
It has two main parts, a \texttt{<genetags>} tag first, and then a
number of \texttt{<gene>} tags, one for each gene in the data set.

Before we continue with the gene tags, we define a new tag type, that
is required to include a \texttt{name} attribute. We call this type
...
The \texttt{<genetags>} tag has one required and many optional
subtags:
\begin{schema}  
...
\end{schema}
The \texttt{<id>} must be an integer number, that starts with one and
goes up to the number of genes. \texttt{<name>} is typically the name
of the probe-set, but be used for other purposes as well,
\texttt{<symbol>} is typically the canonical gene
symbol. \texttt{<entrezid>} is the Entrez gene id. If a given
probeset does not map to a known gene, then \texttt{<symbol>} and
\texttt{<entrezid>} can be set to \texttt{NA}. Finally, the gene tags
might contain any number number of \texttt{<x>} tags, each defining an
...
individual genes. Here is an example gene tags section:
\begin{XML}
<genetags>
  <id>#</id>
  <name>Name</name>
...
\end{XML}
Here we define an extra column for the gene table, this will give the
chromosome on which the gene is located.

After the gene tags, we have a typically large number of genes in the file:
\begin{schema}
...
\end{schema}
These must have exactly the same subtags as the ones given in the gene
tags section. For example:
\begin{XML}
<gene>
  <id>1</id>
...
The samples and their associated data have a similar format to the
genes:
\begin{schema}  
...
\end{schema}
The \texttt{<samples>} tag has two parts, the first defines the sample
meta data entries and the second part contains one \texttt{<sample>}
...
\end{schema}
The \texttt{<id>} and \texttt{<name>} sample tags are
required. \texttt{<id>} contains numeric ids, from one, up to the
number of samples in the data set. \texttt{<name>} is an arbitrary
...
number of additional \texttt{<x>} tags can be added, each with a
unique \texttt{name} attribute, to define additional information about
the samples.

Here is an example \texttt{<sampletags>} tag:
\begin{XML}
<sampletags>
  <id>#</id>
  <name>Name</name>
...
\end{schema}
Here is an example, that corresponds to the \texttt{<sampletags>}
entry given above:
\begin{XML}
<sample>
  <id>1</id>
...
Again, the \texttt{<modules>} tag has a syntax that is similar to the
...
\texttt{<moduletags>}, this defines the meta-data associated with the
modules. The \texttt{<gotags>} and \texttt{<keggtags>} tags are
...
biclusters. Finally, there is a list of \texttt{<module>} tags, one
for each bicluster.
...
\end{schema}
There is only one tag required in \texttt{<moduletags>}, the
\texttt{<id>}, which is a numeric id, going from 1 to the number of
modules. The rest is optional, and it is also possible to create any
kind of extra tags with the \texttt{<x>} tag. \texttt{<name>} is an
arbitrary character string. The other optional tags are
mainly defined to modules coming from the ISA algorithm:
\texttt{<iterations>} gives the number of ISA iterations needed to
find the module, \texttt{<oscillation>} specifies the oscillation
cycle length for oscillating modules. It is mostly zero, meaning that
the module does not oscillate. \texttt{<thr\_row>} is the ISA gene
threshold, \texttt{<thr\_col>} is the ISA condition
threshold. \texttt{<freq>} is the number of ISA seeds that converged
to the module, \texttt{<rob>} is its robustness
score. \texttt{<rob\_limit>} is the robustness threshold that was used
to filter the module.

Here is an example \texttt{<moduletags>} tag:
\begin{XML}
<moduletags>
  <id>#</id>
  <name>Name</name>
  ...
  <thr_row>thr.row</thr_row>
  <thr_col>thr.col</thr_col>
  <freq>freq</freq>
  <rob>rob</rob>
  <rob_limit>rob.limit</rob_limit>
</moduletags>
\end{XML}

The optional \texttt{<gotags>} and \texttt{<keggtags>} tags are
discussed in the next subsection, let us now see how the data for a
...
\texttt{<moduletags>} section above. The others are:
\texttt{<containedgenes>}, a space separated list of gene ids, for the
genes that are included in the module; \texttt{<genescores>}, the gene
scores of these genes, in the order of the list in
\texttt{<containedgenes>}; \texttt{<containedsamples>}, the ids of the
samples in the module; \texttt{<samplescores>}, the scores for these
samples; \texttt{<intersectingmodules>}, the ids of the modules that
have an overlap with the given module. \texttt{<gos>} and
\texttt{<keggs>} are optional and describe the Gene Ontology and KEGG
pathway enrichment of the given module, see their details in the next
subsection.

Here is an example \texttt{<module>} tag:
\begin{XML}
<module>
  <id>1</id>
  <name>module 1</name>
...
\texttt{<moduletags>} is followed by a \texttt{<gotags>} and/or a
\texttt{keggtags} tag:
\begin{schema}
...
\end{schema}
Here \texttt{<id>} is a numeric id, starting from one, up to the total
number of enriched GO categories; \texttt{<go>} is the GO id;
\texttt{term} is the GO term; \texttt{ontology} is the GO ontology of
the term, possible values: \texttt{BP}, \texttt{CC}, \texttt{MF};
\texttt{pvalue} is the enrichment $p$-value; \texttt{oddsratio} is the
odds ratio; \texttt{expcount} is the expected number of genes in the
intersection, by chance; \texttt{count} is the number of genes in the
intersection; \texttt{size} is the size of the GO term, i.e. the
...
\end{schema}
The tags for \texttt{<keggtags>} is almost the same as for
\texttt{<gotags>}, but here \texttt{<kegg>} id the KEGG pathway ID,
and \texttt{<pathname>} is the name of the pathway. The rest is the
same.

Here is part of an EVF file with the \texttt{<gotags>} and
\texttt{<keggtags>} tags:
\begin{XML}
<gotags>
  <id>#</id>
...
The actual enrichment data is given in the \texttt{<module>} tags,
these can contain a \texttt{<gos>} and/or a \texttt{<keggs>} tag with
...
\end{schema}
\texttt{<gos>} is a list of \texttt{<go>} tags.
...
\end{schema}
\texttt{<keggs>} is a list of \texttt{<kegg>} tags.

The subtags within a \texttt{<go>} or a \texttt{<kegg>} tag refer to
the tags already listed in the \texttt{<gotags>} and
\texttt{<keggtags>} tags:
\begin{schema}
...
Here is an example, this is part of a \texttt{<module>} tag:
\begin{XML}
<gos>
  <go>
    <id>1</id>
...
Finally, the expression data in included in the \texttt{<data>}
...
\begin{XML}
<data>
/Acs1hZT+FDeFGH/29zL+3rK4+sTCvzZzxVgvNrM5Bw8Kfjw2vb6Of8qti943/0QIF8xJy7g
zMoOqyYA4/ne1KsZ1bH69u3c28zewtQM9G8T5zGK7P0szwIM/ub2z9zwHxclIdz3ywC6xtfO
DewMklghAfGqzsQtDeHZ3wolAC1GSN+9N+/i5Oz99bTzzRb1QvzHE+Qlr1Ej1U697+AFvtyn
...
</data>
\end{XML}
...
schema file and example data file.
