...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{blockplot}
...
\caption{The robustness scores of ISA modules after filtering out some
  modules with low robustness. It is still obvious that ten modules
  have a significantly higher score than the rest.}
\label{fig:robbox2}
\end{figure}
...
<<bestmods>>=
bestmods <- which(modules3$seeddata$rob > 27.5)
...
them.
<<othermods.pre>>=
png("othermods.png", 800, 800)
@ 
<<othermods>>=
othermods <- which(modules3$seeddata$rob < 27.5)[1:5]
print(plot.modules(modules3, othermods, data=data[[1]]))
@ 
<<othermods.post>>=
dev.off()
@ 
...
\caption{The input matrix and five modules with relatively low
  ...
  in fact two blocks of the input matrix.}
\label{fig:othermods}
\end{figure}
...
\caption{The sensitivity of the ISA in the function of noise. The
  ...
  ISA is very sensitive to relatively large noise levels.
}
\label{fig:sensbox}
\end{figure}
...
\caption{Specificity of the ISA modules, in the function of
  ...
  Specificity for ISA decreases approximately linearly with the
  increase of noise.
}
\label{fig:specbox}
\end{figure}
