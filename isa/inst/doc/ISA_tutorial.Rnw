...
Visualizing overlapping biclusters is quite a challenging task. We
...
Rfunction{isa.unique}, etc.

In fact \Rfunction{plotModules} calls the
...
difference between bicluster and the rest of the matrix, see
...
\centering
\setkeys{Gin}{width=0.6\textwidth}
<<plotclust,fig=TRUE,echo=FALSE,results=hide>>=
<<plotclust.pre>>
@ 
\caption{Bar plots to show the difference between the biclusters and
