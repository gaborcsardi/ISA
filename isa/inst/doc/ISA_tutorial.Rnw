...
less similar rows than in the first case. The same applied to the
...
singular value of the matrix. If there would be no thresholding, then
...
arguments. This function uses random seeding and has three optional
arguments: 
\begin{description}
  \item[\Rargument{thr.row}] A numeric vector, the row thresholds to
    use. By default this is $1,1.5,2,2.5,3$. 
  \item[\Rargument{thr.col}] A numeric vector, the column thresholds
    to use. By default this is $1,1.5,2,2.5,3$.
  \item[\Rargument{no.seeds}] The number of random seed vectors to
    generate. By default 100 seeds are generated.
\end{description}
...
Let us see an example. We load the \Rpackage{isa} generate some in
silico data with the \Rfunction{isa.in.silico} function first:
<<insilico,cache=TRUE>>=
data <- isa.in.silico(200,200,2,50,50)[[1]]
@
The function call above generates the data matrix in
Fig.~\ref{fig:twoclusters}. The first two arguments define the size of
...
\Rfunction{isa.in.silico} returns a list with a list with three
elements, we are most interested in the first now, that is the actual
artificial data matrix. The second and the entries in the list give
...
<<blockplot,include=FALSE,results=hide,cache=TRUE>>=
print(image(Matrix(data)))
@ 

\begin{figure}
\centering
\setkeys{Gin}{width=0.7\textwidth}
<<blockplot2,fig=TRUE,echo=FALSE,results=hide,cache=TRUE,width=8,height=6>>=
<<blockplot>>
@
...
with opposite score signs. The \Rfunction{isa} function finds modules
...
50 rows. This is why they are collected into a single module.
...
contains data about the ISA run, all the parameters (the default
...
various data about the individual modules or the seeds that were used
to find the module. There is one row for each module. It has the
following columns:
<<seedcols>>=
...
parameters of some steps. 

Let us create some in-silico data to be analyzed:
<<insilico10>>=
data <- isa.in.silico(500, 200, 10)
@ 
This will be a $500\times 200$ matrix, with 10 modules. By default
``half'' of the matrix is filled with noise, so each module will be
$25\times 10$ submatrix. See the input matrix on
Fig.~\ref{fig:tenmodules}.

<<tenmods,include=FALSE,results=hide,cache=TRUE>>=
print(image(Matrix(data[[1]])))
@ 

\begin{figure}
\centering
\setkeys{Gin}{width=0.7\textwidth}
<<tenmods2,fig=TRUE,echo=FALSE,results=hide,cache=TRUE>>=
<<tenmods>>
@
...
for modules that are differentially regulated in the control and the
case samples.
...
that did not converge, the latter result all \verb+NA+
...
\centering
\setkeys{Gin}{width=0.7\textwidth}
<<robbox2,fig=TRUE,echo=FALSE,results=hide,cache=TRUE>>=
<<robbox>>
@
\caption{}
\label{fig:robbox}
...
@
We use the very same row seeds, as for the original matrix; this is
not strictly requires. The \Rargument{perms} argument sets the number
...
\centering
\setkeys{Gin}{width=0.7\textwidth}
<<robbox4,fig=TRUE,echo=FALSE,results=hide,cache=TRUE>>=
<<robbox3>>
@
\caption{}
\label{fig:robbox2}
...
<<bestmods>>=
bestmods <- which(modules3$seeddata$rob > 61)
...
and sensitive. Of course for the analysis of real data it is much more
...
also kept after the robustness-based filtering. Here we plot nine of
them.
<<othermods,include=FALSE,results=hide,cache=TRUE>>=
othermods <- which(modules3$seeddata$rob < 61)[1:3]
layout(matrix(seq(length(othermods)+1), 2, byrow=TRUE))
idx <- seq(ncol(data[[1]]), 1)
par(mar=c(1,1,4,1))
image(1-data[[1]][,idx], main="Input matrix", 
      axes=FALSE, frame=TRUE)
for (i in othermods) { 
  par(mar=c(1,1,4,1))
  image(1-outer(modules3$rows[,i], modules3$columns[,i])[,idx],
        main=paste("Module", i), axes=FALSE, frame=TRUE)
}
@ 
...
\centering
\setkeys{Gin}{width=0.7\textwidth}
<<othermods2,fig=TRUE,echo=FALSE,results=hide,cache=TRUE>>=
<<othermods>>
@ 
\end{figure}
...
In this section we show some examples that highlight some key features
...
<<noiseplot1,echo=FALSE,results=hide>>=
png("noise.png", width=800, height=320)
@ 
<<noiseplot2>>=
layout( matrix(1:10, nr=2, byrow=TRUE) )
for (i in seq_along(data)) {
  par(mar=c(1,1,1,1))
  image(1-data[[i]][[1]], axes=FALSE, frame=TRUE)
}
@ 
<<noiseplot3,echo=FALSE,results=hide>>=
dev.off()
@ 
\begin{figure}
  \centering
...
Let's create boxplots for the different noise levels.
...
<<overlapplot,include=FALSE,results=hide,cache=TRUE>>=
layout( rbind(1:4, c(8,5,6,7)) )
par(mar=c(1,1,3,1))
image(2-two.over[[1]], axes=FALSE, frame=TRUE, main="Input matrix")
for (i in 1:3) {
  par(mar=c(1,1,3,1))
  image(1-outer(ov.modules.1$rows[,i]!=0, ov.modules.1$columns[,i]!=0),
        axes=FALSE, frame=TRUE, main=paste("Module", i))
}
for (i in 1:3) { 
  par(mar=c(1,1,3,1))
  image( 1-outer(ov.modules.2$rows[,i]!=0, ov.modules.2$columns[,i]!=0),
        axes=FALSE, frame=TRUE, main=paste("Module", i+3))
}
@ 

\begin{figure}
\centering
<<overlapplot2,fig=TRUE,echo=FALSE,results=hide,cache=TRUE,width=8,height=5>>=
<<overlapplot>>
@ 
\caption{}
...
also the union of them. With the stricter thresholds it find the
...
show and example on how to do this easily with the \Rpackage{snow} and
...
Create the MPI cluster, with eight working nodes. You need to have a
...
invisible(clusterEvalQ(clu, library(isa)))
...
\newblock {\em isa: The Iterative Signature Algorithm}.
