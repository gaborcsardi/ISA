...
depend on the random number generator used, the ISA run is commented out 
and we just load a precomputed set of modules in this vignette.
<<simple ISA>>=
thr.gene <- 2.7
thr.cond <- 1.4
