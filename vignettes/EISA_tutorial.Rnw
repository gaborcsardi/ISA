...
<<heatmap,fig=TRUE>>=
ISA2heatmap(modules, 1, ALL, scale="none")
@ 
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{graphics/heatmap.png}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{graphics/parallel.png}
...
cond.plot(modules, 1, ALL, col=col)
@
...
<<iteration>>=
modules1 <- ISA.iterate(ALL.normed, feature.seeds=random.seeds, 
                        thr.feat=2, thr.samp=2)
modules2 <- ISA.iterate(ALL.normed, sample.seeds=smart.seeds,
                        thr.feat=2, thr.samp=2)
@
