...
The \Rfunction{ISAGO} function requires the annotation package on the
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{graphics/HTMLTable}
...
The Rfunction{ISAHTMLModules} generates a HTML page for every single
module with the following information on it: 
\begin{itemize}
...
module only. The \Rpackage{ExpressionView} package visualizes a set of
possibly overlapping biclusters together.
...
dimensions to the \Rfunction{ISAIterate} function.
