...
input of a biclustering method a matrix and the output is a set of
...
the appropriate annotation package must be installed.
...
The \Rfunction{ISA.GO} function requires the annotation package on the
...
library(GO.db)
mget(sigCategories(GO$BP)[[1]], GOTERM)
@ 
...
manual page of the \Rfunction{ISA.GO} function for details, and also
...
the \Rfunction{ISA.KEGG} function:
<<KEGG enrichment>>=
...
\begin{figure}
\centering
<<goplot2-real-real,width=10,height=5,fig=TRUE,results=hide,echo=FALSE>>=  
<<goplot2-real>>
@
...
the plot. The graph object contains the full name of the category as
...
also want to keep only single probeset for each Entrez gene.
<<Entrez unique>>=
...
expression, the row-wise (=gene-wise) normalized and the column-wise
(=sample-wise) normalized expression matrix.
<<normed>>=
...
As it turns out, one modules found using the smart seeds was also
found with random seeds.
