...
tissues. You are expected to understand the basics of R and ISA
for this tutorial; please see the introductory ISA tutorials
otherwise.
...
loads the implementation of the ISA. The \Rpackage{affy} package
...
process, we perform it only once and store the results in a file. We
don't have to do anything if this file already exists.
...
\Rclass{ExpressionSet}, then we do't have to do anything.
...
samples of the module.) The condition threshold is set to 1 and 2. We
...
\Sexpr{length(unique(tissues[getSamples(modules, mymod)[[1]]]))}
...
regulated the same in all samples of the tissue. We also mark
...
<<condplot3>>
@ 
\caption{Condition plots for four modules that are specific for a
single tissue, modules: \Sexpr{paste(to.plot, collapse=", ")} (from
top to bottom and left to right). They are specific to
...
KEGG pathway. Let's create a good-looking table that lists the tissues
...
topgo <- sapply(seq_len(length(modules)), function(x) {
  ...
  paste(goterm(sigCategories(GO[[mc]], p=1)[[x]][1]), 
...
<<create KEGG>>=
keggterms <- keggpath(sapply(sigCategories(KEGG, p=1), first))
