...
multiplied by $c_n$ and (4) the result is thresholded with
...
We use subsets of the same data set for the test, this is a public
...
any BioConductor packages, so we will use the \Rpackage{eisa}
package.

The non-normalized data set of the experiment is available as numeric
matrix. To spare time and bandwidth, we only download it once, even if
the code of this tutorial is run multiple times. We store the data
file in the current local directory. If the file is already there,
...
start with an exclamation mark in GEO files, so skip them by setting
...
We quickly test this function, on a small subset of out data, with 
...
We are ready to create the plot, the results can be seen in 
Fig.~\ref{fig:by-rows}.
...
\caption{Running time of the ISA, in the function of the number of rows
  in the input matrix, for various row thresholds. Crealy, the running time
  increases linearly with the number of rows, for all row threshold. It is 
...
We create a function to perform the all the runs for a given column
threshold, with replications.
<<change number of columns set up things>>=
...
\caption{Running times, in the function of the number of columns in the 
  ...
  because the row seed matrix is one order of magnitude larger than the 
...
Fianally, we craete the figure, see Fig.~\ref{fig:no-seeds}.
...
\caption{The running time, in the function of the number of starting seeds, 
  for various row and column combinations. Each point is the average of 
