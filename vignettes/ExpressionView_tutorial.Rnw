...
In both cases, the random number generator is initialized manually using \Rfunction{set.seed(1)}, 
...
<<export, eval=FALSE>>=
ExportEV(modules, ALL, optimalorder, filename="file.xml")
@
...
\variable{optimalorder} and adds the biclusters defined in \variable{modules}. Since the output is an uncompressed XML file, it
is helpful to use the appropriate extension. The XML layout is described in the R help file. Before exporting the data, the 
...
href{http://serverdgm.unil.ch/bergmann/software/expressionview}{website}.
