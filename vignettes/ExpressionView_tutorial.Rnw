...
a \Rpackage{Bioconductor} package designed to interactively explore biclusters identified in gene 
...
In this tutorial we will use the \Rpackage{Bioconductor} sample data from a 
...
suitable groups of genes and samples that are co-regulated. In what follows we
show how to use the freely available biclustering methods.
...
<<ISA, cache=TRUE>>=
set.seed(1) # set seed to get the same results, always
modules <- ISA(ALL)
@ 
...
default values, i.e., $thr.gene=c(2, 2.5, 3, 3.5, 4)$ and $thr.cond=c(1,1.5,2,2.5,3)$.
In both cases, the ISA runs from \Sexpr{formals(eisa:::ISA)$no.seeds} random seeds, to
give reproducible results.

The \Rfunction{isa} function returns an \Rclass{ISAModules} object. Typing its name
returns a brief summary of the results:
<<ISAModules summary>>=
modules
@
...
same sign, then they are correlated, if the sign of their scores is
opposite, then they are anti-correlated.
...
to store a set of biclusters. Let us use the Plaid Model Bicluster Algorithm~\cite{turner03}
on the \Rpackage{ALL} data set
<<biclust bcplaid,cache=TRUE>>=
library(biclust)
biclusters <- biclust(exprs(ALL), BCPlaid(), fit.model = ~m + a + b, verbose = FALSE)
@

The standard R \Rfunction{as} function
...
@
to get an \Rclass{ISAModules} class.
...
@
indicating if a given gene $i$ is contained in module $j$ if $modules.genes[i,j]\ne0$.
Using these matrices, it is straightforward to define the \Rclass{ISAModules} variable:
<<toisa>>=
new("ISAModules",
...
The \Rfunction{order.clusters} function implemented in the \Rpackage{ExpressionView}Êpackage
...
@
Here, $i=0$, returns the map of the complete data set. By default, the \Rfunction{order.clusters}
runs for roughly one minute, this might not be sufficient to find the optimal order for data 
containing many overlapping biclusters. The status of the ordering is stored in 
<<status ordering>>=
optimalorder$status
@
If the status is set to 1, the algorithm has found the optimal solution. A 0 indicates that the
the calculation could not be terminated within the given timeframe. The \Rfunction{order.clusters} 
...
optimalorderp <- order.clusters(modules, initialorder=optimalorder, maxtime=120)
@
and one can increase the time limit by specifying \Rargument{maxtime}.
