...
The Iterative Signature Algorithm (ISA)~\cite{isa} is a biclustering
method, that can efficiently find potentially overlapping biclusters
(modules, according to the ISA terminology) in a matrix. The ISA is
implemented in the \Rpackage{eisa} package~\cite{eisapackage} it uses
standard BioConductor classes and includes a number of visualization
tools as well.
...
annotated to involved in immune system processes, according to the Gene
Ontology database.
...
some \Rpackage{eisa} functions, we add the name of the annotation package to 
the parameters stored in the \Rclass{Biclust} object, this is always adwised.
...
Doing some enrichment analysis is easy:
<<biclust enrichment>>=
library(KEGG.db)
...
\caption{Heatmap of the second module, found with the Plaid Model
  ...
  all samples in the module belong to T-cell samples.}
\label{fig:heatmap}
\end{figure}
