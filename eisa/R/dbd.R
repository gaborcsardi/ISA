...
setMethod("makeValidParams", "DBDListHyperGParams",
          function(object) {
            ## TODO: proper checks
...
setMethod("drive", signature("DBDListHyperGParams"), function(r) { r@drive })
setReplaceMethod("drive", c("DBDListHyperGParams", "logical"),
                 function(r, value) {
                     if (is.na(value))
...
setMethod("categoryToEntrezBuilder",
         signature(p="DBDListHyperGParams"),
         function(p) {
...
## have at least one DBD annotation
...
isa.DBDListHyperGTest <- function(p) {
  p <- makeValidParams(p)
  ...
  ## We need the reverse mapping, the Entrez ids for all DBD
...
setMethod("hyperGTest",
          signature(p="DBDListHyperGParams"), isa.DBDListHyperGTest)

setMethod("show", signature(object="DBDListHyperGParams"),
          function(object) {
...
setMethod("show", signature(object="DBDListHyperGResult"),
          function(object) {
...
setMethod("summary", signature(object="DBDListHyperGResult"),
...
setMethod("htmlReport", signature=(r="DBDListHyperGResult"),
...
setMethod("pvalues", signature(r="DBDListHyperGResult"),
          function(r) lapply(r@reslist, function(x) {
            structure(x$Pvalue, names=rownames(x))
          }))

setMethod("geneCounts", signature(r="DBDListHyperGResult"),
          function(r) lapply(r@reslist, function(x) {
            structure(x$Count, names=rownames(x))
          }))

setMethod("oddsRatios", signature(r="DBDListHyperGResult"),
          function(r) lapply(r@reslist, function(x) {
            structure(x$OddsRatio, names=rownames(x))
          }))

setMethod("expectedCounts", signature(r="DBDListHyperGResult"),
          function(r) lapply(r@reslist, function(x) {
            structure(x$ExpCount, names=rownames(x))
          }))

setMethod("universeCounts", signature(r="DBDListHyperGResult"),
          function(r) lapply(r@reslist, function(x) {
            structure(x$Size, names=rownames(x))
          }))

setMethod("universeMappedCount", signature(r="DBDListHyperGResult"),
          function(r) length(r@universeGeneIds))

setMethod("geneMappedCount", signature(r="DBDListHyperGResult"),
          function(r) sapply(r@geneIds, length))

setMethod("geneIdUniverse", signature(r="DBDListHyperGResult"),
          function(r, cond=FALSE) r@catToGeneId)

setMethod("condGeneIdUniverse", signature(r="DBDListHyperGResult"),
          function(r) geneIdUniverse(r, cond=TRUE))

## This function gives all the hits for the tested categories.
setMethod("geneIdsByCategory", signature(r="DBDListHyperGResult"),
...
setMethod("sigCategories", signature(r="DBDListHyperGResult"),
          function(r, p) {
...
isa.DBD <- function(isaresult,
  ...
  isa:::isa.status("Calculating DBD enrichment", "in")
  ...
  params <-
    try( new("DBDListHyperGParams", geneIds = selectedEntrezIds,
...
convert.dbd <- function(file, org="Mm") {  
  ...
  len <- len[ sapply(len, function(x) !is.na(len[1])) ]
  len <- sum(sapply(len, length))
  tab2 <- matrix(nr=len, nc=2)
  ptr <- 1
  for (i in seq_len(nrow(tab))) {
    eg <- map[[as.character(tab[i,1])]]
    if (is.na(eg[1])) { next }
    new.rows <- cbind(eg, as.character(tab[i,2]))
    tab2[ ptr:(ptr+NROW(new.rows)-1), ] <- new.rows
    ptr <- ptr + NROW(new.rows)
    if (! i %% 1000) print(i)
  }
