...
to store a set of biclusters. The \Rfunction{eisa.biclust} function converts ISA
...
@ 
<<parallel>>=
ex <- samp.exprs(isa.normalize(ALL[featureNames(modules),]))
parallelCoordinates(ex, Bc, number=2, pch=NA)
@
