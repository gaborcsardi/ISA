...
We also need to add the input data to the \Rclass{Biclust} object by
hand, as this is not included in the \Rclass{Biclust} object, when
using a recent version of the \Rpackage{biclust} package. The column
and row names of the data will be used later, e.g. for the enrichment
analysis. The conversion itself can be performed with the usual
\Rfunction{as} function.  
<<convert bc to isa>>=
Bc@Parameters$annotation <- annotation(ALL.filtered)
Bc@Parameters$Data <- exprs(ALL.filtered)
