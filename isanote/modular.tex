...
to run the ISA, visualize its output and puts the biclusters into
biological context.
...
\caption{\subfig{(A)} Work flow of a typical modular analysis study, with the
  \Rpackage{eisa} package. See the text for details.
...
additional data normalization is vital to reduce the bias due to
...
In each iteration of the ISA thresholded weighted sums of expression
levels over either genes or samples are computed. Since different genes
...
The ISA iteration is typically performed to a
...
seeds a biased to start with certain sets of genes or samples based on
prior knowledge.As mentioned previously, ISA can be performed with random or smart
seeds, based on the application.

% Merging the modules.
\emph{Merging the modules.} (Step 3.)
...
\emph{Robustness of biclusters.} (Step 3.)
